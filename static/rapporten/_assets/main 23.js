...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll("#issues > article.issue").forEach((article) => {
    ...
    h3s[0].before(headerRow);

    h3s.forEach((h3) => {
      // Collect following siblings until next h3
      const content = [];
      let sibling = h3.nextSibling;
      while (sibling) {
        if (sibling.nodeType === 1 && sibling.tagName === "H3") break;
      ...
      // Extract meta info
      const metaDiv = content.find(
        (el) => el.nodeType === 1 && el.classList && el.classList.contains("meta")
      );
      let impact = "";
      let type = "";
      let beperking = "";
