...
(function () {
  ...
  var LANG = {
    nl: {
      ...
      // Image checks
      imgNa: "Afbeeldingen niet controleerbaar",
      imgNaDesc: "Omdat het document geen tags heeft, kunnen afbeeldingen niet gecontroleerd worden.",
      imgNone: "Geen afbeeldingen gevonden",
      imgNoneDesc: "Er zijn geen Figure-tags gevonden in dit document.",
      imgAiAlt: "Automatisch gegenereerde alt-tekst",
      imgAiAltDesc: "Een of meer afbeeldingen hebben een automatisch gegenereerde alt-tekst (herkenbaar aan 'content generated by AI'). Zulke teksten zijn meestal niet bruikbaar. Schrijf zelf een alt-tekst die de functie van de afbeelding beschrijft.",
      ...
      // List checks
      lstNa: "Lijsten niet controleerbaar",
      lstNaDesc: "Omdat het document geen tags heeft, kunnen lijsten niet gecontroleerd worden.",
      ...
      // Table checks
      tblNa: "Tabellen niet controleerbaar",
      tblNaDesc: "Omdat het document geen tags heeft, kunnen tabellen niet gecontroleerd worden.",
      ...
      disclaimerItems: [
        ...
        "Of tekst die visueel als kop is vormgegeven ook als kop is getagd",
        "Of de ingestelde documenttaal overeenkomt met de werkelijke taal van de inhoud",
    ...
    en: {
      ...
      imgNa: "Images cannot be checked",
      imgNaDesc: "Because the document has no tags, images cannot be checked.",
      imgNone: "No images found",
      imgNoneDesc: "No Figure tags were found in this document.",
      imgAiAlt: "Automatically generated alt text",
      imgAiAltDesc: "One or more images have an automatically generated alt text (recognizable by 'content generated by AI'). Such texts are usually not useful. Write an alt text yourself that describes the function of the image.",
      ...
      lstNa: "Lists cannot be checked",
      lstNaDesc: "Because the document has no tags, lists cannot be checked.",
      ...
      tblNa: "Tables cannot be checked",
      tblNaDesc: "Because the document has no tags, tables cannot be checked.",
      ...
      disclaimerItems: [
        ...
        "Whether text that is visually styled as a heading is also tagged as a heading",
        "Whether the set document language matches the actual language of the content",
  ...
  // The file input is visually hidden but keyboard-focusable. Show a visible
  // focus ring on the "Choose a file" button while the input has focus.
  var uploadBtnEl = uploadZone.querySelector(".tool-pdf__upload-btn");
  if (uploadBtnEl) {
    fileInput.addEventListener("focus", function () {
      uploadBtnEl.classList.add("tool-pdf__upload-btn--focus");
    });
    fileInput.addEventListener("blur", function () {
      uploadBtnEl.classList.remove("tool-pdf__upload-btn--focus");
    });
  }
  ...
  function buildAllChecks(metadata, markInfo, collector, outline) {
    ...
    allChecks.push({ catKey: "catNavigation", checks: checkNavigation(outline) });
    return allChecks;
  }
  ...
  function checkDocument(metadata, markInfo, collector) {
    ...
    // 3. Language
    // Most reliable source: the catalog /Lang entry, which PDF.js exposes as
    // info.Language. PDF.js resolves indirect references and decodes hex /
    // UTF-16BE strings, so this works regardless of how /Lang is stored.
    var declaredLang = null;
    var langSource = "tag";
    if (typeof info.Language === "string" && info.Language.trim().length > 0) {
      declaredLang = info.Language.trim();
    }
    // Fallback 1: /Lang on the structure tree root (rare, page-level)
    if (!declaredLang && collector.rootLang) {
      declaredLang = collector.rootLang;
    }
    // Fallback 2: XMP metadata dc:language (may be a string or an array)
    if (!declaredLang && metadata && metadata.metadata) {
      try {
        var metaLang = metadata.metadata.get("dc:language");
        if (metaLang && typeof metaLang !== "string" && metaLang.length > 0) {
          metaLang = metaLang[0];
        }
        if (typeof metaLang === "string" && metaLang.trim().length > 0 && metaLang.trim().toLowerCase() !== "x-default") {
          declaredLang = metaLang.trim();
          langSource = "meta";
        }
      } catch (e) { /* */ }
    }
  ...
  function checkImages(collector) {
    ...
    var withAltFindings = [];
    var aiAltFindings = [];
    var aiAltPattern = /generated by ai|gegenereerd door ai|door ai gegenereerde/i;
    for (var i = 0; i < figures.length; i++) {
      ...
      if (fig.hasAlt) {
        ...
        if (aiAltPattern.test(fig.altText)) {
          aiAltFindings.push({
            page: fig.page,
            element: "Figure",
            detail: '\u201C' + truncate(fig.altText, 60) + '\u201D'
    ...
    if (aiAltFindings.length > 0) {
      checks.push({
        status: "warn",
        title: t("imgAiAlt"),
        description: t("imgAiAltDesc"),
        findings: aiAltFindings
      });
    }
  ...
  function checkNavigation(outline) {
    var checks = [];
  ...
  function renderCategory(cat) {
    if (!cat.checks || cat.checks.length === 0) return "";
  ...
  function renderCheck(check) {
    ...
    if (hasAccordion) {
      ...
      if (check.fix) {
        ...
        html += '<h4 class="tool-pdf__fix-label">' + t("fixLabel") + '</h4>';
  ...
  function exportFindings(allChecks, fileName) {
    ...
    for (var c = 0; c < allChecks.length; c++) {
      var cat = allChecks[c];
      if (!cat.checks || cat.checks.length === 0) continue;
