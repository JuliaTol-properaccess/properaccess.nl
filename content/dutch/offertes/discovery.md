---
title: "Offerte Digitale Toegankelijkheid Discovery Museum"
date: 2024-02-16
draft: false
noindex: true
robots: "noindex, nofollow"
sitemap_exclude: true
_build:
  list: never
  render: true
offerte_voor: "Discovery Museum"
website_app: "Website"
url_klant: "https://discoverymuseum.nl"
datum_offerte: "18 februari 2026"
offertenummer: "202602-01"
---

{{< protected-section token="DM123" >}}

{{< offerte-section title="Inleiding" >}}

Beste Els,

Wat fijn dat Discovery Museum werk maakt van digitale toegankelijkheid! Als wetenschaps- en technologiemuseum in Kerkrade bereik je een breed publiek — van gezinnen tot scholen en wetenschapsliefhebbers. Het is geweldig dat je iedereen dezelfde kans wilt geven om jullie collectie, evenementen en bezoekersinfo online te ontdekken.

Jullie vraag is helder: waar staan we nu, en welke stappen zijn nog nodig om te voldoen aan de Europese Toegankelijkheidswet? Precies daar helpen wij je graag mee. Onze audit geeft je niet alleen inzicht in wat er speelt, maar ook concrete handvatten om ermee aan de slag te gaan. Onze rapporten staan bomvol begrijpelijke tips en oplossingen, zodat jouw team direct kan starten met verbeteringen.

In dit document vind je een offerte voor de audit van discoverymuseum.nl. Ik hoop dat ons aanbod je aanspreekt. Mijn team gaat heel graag voor je aan de slag!

Hartelijke groet,
**Julia Tol**
CEO Proper Access, senior WCAG-auditor

{{< /offerte-section >}}

{{< offerte-section title="Doel van de audit" subtitle="Wat levert het op?" bg="light" >}}

Onze audit helpt Discovery Museum op drie manieren:

### 1. Aantoonbare naleving van de EAA

Het toegankelijkheidsrapport fungeert als objectief bewijsstuk waaruit blijkt dat de organisatie actief invulling geeft aan haar wettelijke verplichtingen onder de EAA.

### 2. Onderbouwd plan van aanpak richting toezichthouder

Op basis van de analyse wordt een concreet en uitvoerbaar plan van aanpak opgesteld. Dit plan laat zien welke maatregelen worden genomen, binnen welke termijn en met welke prioriteit.

### 3. Beter imago en groter bereik

Door de website daadwerkelijk toegankelijk te maken, wordt deze bruikbaar voor een bredere doelgroep. Dit draagt bij aan een inclusiever imago, verhoogt de gebruiksvriendelijkheid en vergroot het potentiële bereik.

{{< /offerte-section >}}

{{< offerte-section title="Product" subtitle="Hoe we helpen" >}}

### Analyse & bevindingen

- **Duidelijk inzicht in knelpunten**, inclusief visuele voorbeelden (screenshots en indien relevant video-opnames)
- **Knelpunten beschreven per pagina en per element** (uniek formaat in Nederland!)
- **Uitleg van impact op gebruikers** (groot, medium, klein) en type fout (content of techniek)
- **Per bevinding het relevante succescriterium** van WCAG en EN 301 549

### Onderzoeksopzet & testmethoden

- **Handmatige controle** op ca. 200 controlepunten per pagina in de steekproef
- **Testen in donkere en lichte modus**
- **Browsers:** laatste versies van Google Chrome, Firefox en Safari
- **Contrast getest** in dark mode en licht mode
- **Screenreaders:** VoiceOver (Mac) en NVDA (Windows)
- **6-ogenprincipe:** elke bevinding gecontroleerd door twee senior onderzoekers
- **Rapport kan desgewenst ook in het Engels worden opgeleverd**

### Oplossingsrichtingen

- **Praktische oplossingen per bevinding**, waar mogelijk uitgewerkt in HTML, CSS en JS of in pseudocode
- **Voorstel voor alternatieve oplossingen**
- **We kijken verder dan de WCAG** — kansen voor extra toegankelijkheid of gebruiksvriendelijkheid nemen we mee
- **Duidelijk onderscheid** tussen WCAG-issues en aanvullende adviezen

### Plan van aanpak

- **Export bevindingen als CSV-bestand**
- **Volledig overzicht** van alle te ondernemen acties in online spreadsheet
- **Filteren** op impact, type probleem, WCAG-succescriterium
- **Bruikbaar als verantwoording** richting toezichthouder én voor interne planning

{{< /offerte-section >}}

{{< offerte-section title="Audit vs. tool" subtitle="Waarom menselijke expertise?" bg="light" >}}

Een automatische toegankelijkheidstool lijkt verleidelijk, maar detecteert slechts 20-30% van alle toegankelijkheidsproblemen. Veel cruciale aspecten — zoals de kwaliteit van alt-teksten, logische leesvolgorde, toetsenbordnavigatie en screenreaderondersteuning — kunnen alleen door ervaren menselijke auditors worden beoordeeld.

Bovendien bieden zogenaamde 'overlay-tools' vaak een schijnoplossing die de gebruikservaring voor mensen met een beperking juist verslechtert. Een menselijke audit geeft je betrouwbaar inzicht en werkbare oplossingen.

Meer hierover lees je op: [properaccess.nl/overlay-toegankelijkheidstools/](https://properaccess.nl/overlay-toegankelijkheidstools/)

{{< /offerte-section >}}

{{< offerte-section title="Offerte" subtitle="Investering" >}}

| Product                                                                                              | Prijs (excl. BTW) |
| ---------------------------------------------------------------------------------------------------- | ----------------- |
| Audit digitale toegankelijkheid van de website inclusief onbeperkt vragen stellen n.a.v. het rapport | € 2.695           |
| Retest van bevindingen                                                                               | € 500             |
| Nabespreking van het rapport online, 1 uur                                                           | € 125             |

**Totaal:** € 3.320 (excl. BTW)

**Op alle producten en diensten is 21% BTW van toepassing.**

{{< /offerte-section >}}

{{< offerte-section title="Planning" bg="light" >}}

De audits worden in overleg ingepland. We kunnen binnen 3 tot 5 weken na akkoord starten en binnen 4 weken na de start het resultaat opleveren.

{{< /offerte-section >}}

{{< offerte-section title="Over ons" >}}

### Fijne partner

Onze klanten waarderen onze warme samenwerking, inlevingsvermogen en begrijpelijke rapporten. We communiceren helder en denken actief mee over praktische oplossingen.

### Kennis van de regels

Proper Access heeft jarenlange samenwerking met DigiToegankelijk, draagt bij aan NL/VL overleggen en is betrokken geweest bij DigiToegankelijk TOP (Logius) 2022-2026.

### Vertrouwelijkheid

Vertrouwelijke informatie achter logins komt niet in het rapport. We hanteren strikte privacy- en beveiligingsprotocollen.

### Algemene voorwaarden

Op onze diensten zijn de Algemene Inkoopvoorwaarden VNG GIBIT 2023 van toepassing.

### Betaling

Factuur na afronding van de audit, betaaltermijn 30 dagen.

{{< /offerte-section >}}

{{< offerte-section title="Voorbeeld-rapporten" subtitle="Bekijk onze kwaliteit" bg="light" >}}

Wil je een indruk krijgen van de kwaliteit en diepgang van onze rapporten? Bekijk deze voorbeelden:

- [NRC rapport](https://www.properaccess.nl/rapporten/202512_nrc/)
- [Rijksmuseum iOS-app](https://www.properaccess.nl/rapporten/202512_iOS_Rijks/)

{{< /offerte-section >}}

{{< offerte-section title="Contact" >}}

**Proper Access B.V.**
Willemserf 20
3755 XK Eemnes

**Telefoon:** 06-28742274
**E-mail:** contact@properaccess.nl
**Website:** [properaccess.nl](https://properaccess.nl/)

**BTW-nummer:** NL867096755B01
**KVK-nummer:** 95350985

**Directeur:** Julia Tol

{{< /offerte-section >}}

<div class="offerte-cta">
  <div class="container">
    <p>Heb je vragen over deze offerte of wil je een toelichting? Neem gerust contact op. We horen graag van je!</p>
    <p><strong>Julia Tol</strong><br>CEO Proper Access</p>
  </div>
</div>

{{< /protected-section >}}
