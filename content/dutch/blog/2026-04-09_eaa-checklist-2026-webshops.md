...
Zet een contrast-check aan op je pagina, bijvoorbeeld met onze gratis WCAG Radar op /tools/wcag-radar/ en test je hoofdtekst, je knop-tekst, je linkkleur, je placeholder-tekst en je foutmeldingen. Lichtgrijze tekst op witte achtergrond is een klassieker, bijna altijd te laag.
...
Eén `<h1>` per pagina, dan `<h2>` voor de secties, dan `<h3>` voor sub-secties. Geen `<h1>` gevolgd door `<h4>`, geen `<h2>` gebruikt voor opmaak. Je homepage en productpagina zijn de plekken waar dit het vaakst fout gaat. Test de koppen met de gratis [WCAG Radar van Proper Access](/tools/wcag-radar/).
