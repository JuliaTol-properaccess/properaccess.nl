...
PA.register({
  ...
  run: function (ctx) {
    function lmName(el) {
      ...
      if (lb) { var t = document.getElementById(lb.split(/\s+/)[0]); if (t) return (t.textContent || "").trim(); }
      return "";
    }
    var explicit = { main: "main", navigation: "navigation", complementary: "complementary",
      banner: "banner", contentinfo: "contentinfo", region: "region", search: "search", form: "form" };
    var nodes = Array.prototype.slice.call(document.querySelectorAll("main,nav,aside,header,footer,section,form,[role]"));
    var found = [], mains = 0, byRole = {};
    nodes.forEach(function (el) {
      if (el.closest("[data-pa-lens]")) return;
      if (!PA.visible(el)) return;
      ...
      var role = null;
      if (explicit[explicitRole]) role = explicit[explicitRole];
      else if (tag === "main") role = "main";
      else if (tag === "nav") role = "navigation";
      else if (tag === "aside") role = "complementary";
      else if (tag === "header") role = el.closest("article,aside,main,nav,section") ? null : "banner";
      else if (tag === "footer") role = el.closest("article,aside,main,nav,section") ? null : "contentinfo";
      else if (tag === "section") role = lmName(el) ? "region" : null;
      else if (tag === "form") role = lmName(el) ? "form" : null;
      if (!role) return;
      if (role === "main") mains++;
      byRole[role] = (byRole[role] || 0) + 1;
      found.push({ el: el, role: role, name: lmName(el) });
    });
    found.forEach(function (f) {
      var dupUnnamed = byRole[f.role] > 1 && !f.name;
      var label = f.role + (f.name ? ": " + f.name : "") + (dupUnnamed ? " (meerdere zonder naam)" : "");
      ctx.mark(f.el, { status: dupUnnamed ? "warn" : "info", label: label });
    });
    var msg = found.length + " landmarks. ";
    msg += mains === 1 ? "Eén main. " : (mains === 0 ? "Geen main-landmark gevonden. " : mains + " keer main (hoort er één te zijn). ");
    var dup = Object.keys(byRole).filter(function (r) {
      return byRole[r] > 1 && found.some(function (f) { return f.role === r && !f.name; });
    });
    msg += dup.length ? "Let op: meerdere " + dup.join(", ") + " zonder onderscheidende naam." : "Landmarks met dezelfde rol hebben een naam of komen één keer voor.";
    return { count: found.length, summary: msg };
  },
  clear: function (ctx) { ctx.clearMarks(); }
});
