...
// Lazy-load: de zoekindex (searchindex.json, ~800 KB) wordt pas opgehaald als de
// bezoeker daadwerkelijk gaat zoeken, niet op elke paginalading.
let indexLoaded = false;
let indexLoading = null;
const ensureIndexLoaded = async () => {
  if (indexLoaded) return jsonData;
  if (!indexLoading) {
    indexLoading = loadJsonData().then((d) => {
      indexLoaded = true;
      return d;
    });
  }
...
if (hasSearchWrapper) {
  ...
  searchInput.forEach((el) => {
    // Preload de index zodra het zoekveld focus krijgt, nog voor het eerste teken.
  ...
  // Deep link met ?s=zoekterm: index direct laden en meteen zoeken.
  if (urlSearchString !== null) {
    ensureIndexLoaded().then(() => doSearch(searchString));
  }
